...
## УДАР ИЗ КАМНЯ. Четыре разных, и каждый раз берётся не тот, что в прошлый.
##
## Играющий (21.09): «часто пугаются не монстра, который резко вышел, а звука,
## которого они не ждут с монстром; лучше сразу несколько, чтобы игрок не мог
## привыкнуть к паттерну». Это ровно так и работает: один и тот же удар на
## третий раз перестаёт быть ударом и становится уведомлением.
##
## Собрано из НАСТОЯЩИХ записей, слоями. Синтезом такое уже пробовали дважды —
## «тихая нота пианино», потом «бум» на синусах, — и оба раза выходила фигня
## (об этом отдельно написано в Sfx._sting). Здесь каждый вариант — это резкий
## верх, мясо в середине и тяжёлый низ, взятые из разных банков и сдвинутые по
## тону так, чтобы не узнавались как знакомый звук игры.
##
## «удар» — это ещё и настоящий записанный стинг поверх (sting_face и другие).
const SCARE_MIX := [
	# Хруст: сухой верх и то, чем режут, — самый «костяной» из четырёх.
	{"слои": [["hit_hi", 6.0, 1.28], ["sever", 3.0, 0.80], ["hit_mid", 2.0, 1.15]],
		"удар": "угол"},
	# Визг: крик, поднятый почти на октаву и оттого нечеловеческий.
	{"слои": [["scream", 4.0, 1.48], ["hit_mid", 5.0, 1.05], ["slice", 1.0, 1.3]],
		"удар": "лицо"},
	# Обвал: низ и камень. Бьёт телом, а не ухом.
	{"слои": [["hit_low", 7.0, 0.70], ["scrape", 6.0, 0.55], ["strain", 2.0, 1.2]],
		"удар": "погоня"},
	# Вдох у самого уха: тише остальных и страшнее всех — потому что тише.
	{"слои": [["breath", 8.0, 0.52], ["hit_hi", 0.0, 1.62], ["skit", 4.0, 0.7]],
		"удар": ""},
]
...
func _update_mon_lamp() -> void:
	...
	# ТЕМНЕЕ, ЧЕМ БЫЛО. Прежние 1 + 9k² делали из твари подсвеченную модель:
	# на двенадцати метрах она читалась зелёным комом. Свет оставляем только на
	# последние метры — дальше пусть работает очертание, а не подсветка.
	# И ЛУЧ ПО НЕЙ СИЛЬНЕЕ. Раньше форму держало её собственное свечение, и
	# лампе доставалась вспомогательная роль. Теперь свечения нет: всё, по чему
	# её видно, — это блик от этого луча на мокрой плёнке. Значит луч и есть
	# единственный источник формы, и он должен доставать.
	var e: float = 0.5 + 6.5 * k * k
...
## Сыграть удар. Никогда тот же, что в прошлый раз: привычка к звуку убивает
## его быстрее, чем громкость спасает.
func _scare_sting(at: Vector3) -> void:
	if sfx == null or SCARE_MIX.is_empty():
		return
	var i: int = _rng.randi() % SCARE_MIX.size()
	if i == _scare_last and SCARE_MIX.size() > 1:
		i = (i + 1 + _rng.randi() % (SCARE_MIX.size() - 1)) % SCARE_MIX.size()
	_scare_last = i
	var v: Dictionary = SCARE_MIX[i]
	for слой in v["слои"]:
		# Слои идут В ГОЛОВУ, а не из точки: удар должен ударить, а не
		# донестись. Направление даёт рёв, который звучит отдельно.
		sfx.play_tone(str(слой[0]), float(слой[1]), float(слой[2]))
	if str(v["удар"]) != "":
		sfx.sting(str(v["удар"]), -2.0)
	print("[журнал] %.0f с: удар из камня №%d" % [_clock, i + 1])
...
func _on_burst_out() -> void:
	if sfx != null:
		...
		# приходит в тишину.
		if jump_now:
			_scare_sting(monster.global_position)
	if player_node != null:
...
func _on_lunged(point_blank: bool) -> void:
	...
	if point_blank:
		if sfx != null:
			# Удар — каждый раз другой (см. _scare_sting), а рёв из точки даёт
			# направление: звук бьёт в голову, а понимание приходит с рёвом.
			_scare_sting(monster.global_position)
