...
## УДАРЫ ИЗ КАМНЯ. Их четыре, и подряд не должно приходить два одинаковых.
## Слышать я их не могу, а вот проверить, что каждый слой действительно звучит
## и что варианты не повторяются, — могу.
func scene_scare() -> void:
	say("═══ УДАРЫ ИЗ КАМНЯ ═══")
	if w.sfx == null:
		return
	w.sfx.watch = true
	var было: int = w.sfx.heard.size()
	var подряд: int = 0
	var прошлый: String = ""
	for i in 12:
		var с: int = w.sfx.heard.size()
		w._scare_sting(w.player_node.global_position)
		await w.get_tree().process_frame
		var слои: Array = []
		for j in range(с, w.sfx.heard.size()):
			слои.append(str(w.sfx.heard[j]["имя"]))
		var ключ: String = ",".join(слои)
		if ключ == прошлый:
			подряд += 1
		прошлый = ключ
		if i < 4:
			say("  удар %d: %s" % [i + 1, ключ])
		if слои.size() < 3:
			warn("в ударе всего %d слоёв: %s — часть банков не нашлась" % [
				слои.size(), ключ])
	w.sfx.watch = false
	say("сыграно ударов 12, всего звуков %d, повторов подряд %d" % [
		w.sfx.heard.size() - было, подряд])
	if подряд > 0:
		warn("два одинаковых удара подряд: к такому привыкают")
...
func scene_in_stone() -> void:
	...
	if m == null:
		return
	set_phase(2)
	# ИГРОК ОБЫЧНЫЙ, А НЕ БЕССМЕРТНЫЙ. С неуязвимостью тварь доходит на свою
	# дистанцию и ЗАВИСАЕТ: схватить не может, а нырнуть ей мешает то же самое
	# расстояние. Замер показывал 73% времени на виду — и мерил не погоню, а
	# это зависание. Живой игрок в этот момент оказывается в хвате и вырывается.
	p.invuln = 0.0
	m.drop_hold()
	m.form_hold = 0.0
	m.form_t = 0.0
	...
	m.dive_t = 0.5
	# «ОН УЖЕ ВЫХОДИЛ». Без этого срабатывает правило первой фазы: игрок подошёл
	# к его стене — он показывает глаза и уползает на ДАЛЬНЮЮ стену карты. В
	# игре это правильно, а в замере погони означало, что через три секунды он
	# в ста девяноста метрах и мерить нечего. Ровно это стенд и показывал.
	m.first_out = false
	m.allow_emerge = true
	m.scare_cool = 999.0
	...
	var потеряли: float = 0.0
	var хватов: int = 0
	var пропал_в_хвате: int = 0
	var близко: int = 0
	var далеко: int = 0
	var живой: float = 0.0
	...
	while t < 30.0:
		...
		# Схватил — вырываемся сразу и меряем дальше: нас занимает ритм погони,
		# а не то, как бот жмёт кнопки в хвате.
		if w.grab_ui != null and w.grab_ui.visible:
			хватов += 1
			# И ПРОВЕРКА ЖАЛОБЫ: он не должен исчезать из собственной атаки.
			if m.mode == "inwall" or not m.visible:
				пропал_в_хвате += 1
			w.grab_ui._end(true)
		# ЖИВАЯ ПОГОНЯ — ЭТО КОГДА ОН РЯДОМ. Если тварь в полусотне метров,
		# меряется уже не погоня, а прогулка: такие кадры в счёт не идут.
		if m._flat_dist(p.global_position) > 26.0:
			потеряли += dt
			if потеряли > 4.0:
				say("    он отстал на %.0f м — замер прерван. Игрок в %s, тварь в %s, режим %s" % [
					m._flat_dist(p.global_position),
					str(w.world_to_cell(p.global_position)),
					str(w.world_to_cell(m.global_position)), m.mode])
				break
			continue
		потеряли = 0.0
		...
		if v:
			видно += dt
			if не_был_виден_ранее(был_виден):
				выходов += 1
				var dd: float = m._flat_dist(p.global_position)
				if dd < m.CATCH_DIST * 1.3:
					вплотную += 1
					близко += 1
				elif dd > 9.0:
					далеко += 1
	...
	w.sfx.watch = false
	if живой < 5.0:
		warn("живой погони набралось %.0f с — мерить нечего" % живой)
		_run_away = false
		return
	say("за %.0f с живой погони: на виду %.0f%%, выходов %d (вплотную %d, дальше девяти метров %d), хватов %d, звуков из стен %d" % [
		живой, видно / живой * 100.0, выходов, вплотную, далеко, хватов,
		звуков_в_камне])
	if пропал_в_хвате > 0:
		warn("исчез из собственной атаки %d раз" % пропал_в_хвате)
	if далеко > выходов * 0.35 and выходов > 2:
		warn("слишком часто вылезает там, где не достаёт: %d из %d" % [далеко, выходов])
	if видно / живой > 0.55:
		warn("на виду больше половины погони — замысел был обратный")
	if выходов < 1:
		warn("за всю погоню не вылез ни разу — так она станет пустой")
...
## Мелкая обёртка ради читаемости условия выше.
func не_был_виден_ранее(было: bool) -> bool:
	return not было
...
func _run_away_loop() -> void:
	# УХОДИМ ШАГ ЗА ШАГОМ, А НЕ В ДАЛЬНИЙ УГОЛ КАРТЫ.
	#
	# Первый заход бегал в случайный тупик — и бот раз за разом проходил СКВОЗЬ
	# тварь. Второй брал самый дальний тупик — и через двадцать секунд бот был
	# в ста семидесяти метрах, где никакой погони уже нет. Оба замера мерили
	# что угодно, кроме погони.
	#
	# Человек в погоне делает простое: сворачивает в ту сторону, где монстра
	# нет, и повторяет это каждые несколько шагов. Так и здесь: каждые полсекунды
	# выбираем соседнюю клетку, которая ДАЛЬШЕ от твари, и идём в неё.
	while _run_away:
		var pc: Vector2i = w.world_to_cell(w.player_node.global_position)
		var лучшая: Vector2i = pc
		var bd: float = -1.0
		for d in [Vector2i(1, 0), Vector2i(-1, 0), Vector2i(0, 1), Vector2i(0, -1)]:
			var n: Vector2i = pc + d
			if w.maze.is_wall(n.x, n.y):
				continue
			var far: float = w.cell_to_world(n, 0.0).distance_to(
				w.monster.global_position)
			if far > bd:
				bd = far
				лучшая = n
		if лучшая != pc:
			await goto_cell(лучшая, 2.5, true)
		else:
			await w.get_tree().create_timer(0.3).timeout
