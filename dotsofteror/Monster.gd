...
## ПОГОНЯ ИДЁТ В КАМНЕ, А НЕ ПО КОРИДОРУ.
##
## Раньше она бежала за тобой на виду и изредка ныряла: восемь секунд видно,
## секунду нет. Это давало игроку ровно то, чего давать не надо, — возможность
## следить за ней глазами и считать расстояние. Играющий предложил перевернуть:
## «пусть всю погоню сидит в стенах, чтобы игрок не знал, с какой стороны на
## него охота, и вылазит только когда на расстоянии удара».
##
## Соотношение теперь обратное: две секунды на виду, пять в камне, и выходит
## она в четырёх случаях из пяти ВПЛОТНУЮ — то есть выход и есть удар. Всё
## время, пока её не видно, из стен вокруг идёт скрежет (см. world._update_lurk):
## слышно, что охота идёт, и не слышно, откуда.
## СКОЛЬКО ОНА НА ВИДУ. Секунда-две оказалась ерундой: играющий (21.09) сказал
## прямо — «появляется на таком расстоянии, что не достаёт, и сразу пропадает».
## Выход должен успеть стать угрозой: три-пять секунд — это подойти и ударить.
const DIVE_GAP := [3.0, 5.5]
const DIVE_HIDE := [5.0, 9.0]    ## и столько её нет нигде
## Доля выходов ВПЛОТНУЮ. На 0.8 проход стенда дал девятнадцать хватов вместо
## десяти: каждый выход оборачивался ударом, и погоня превращалась в череду
## хватов. Чуть больше половины — и у выхода остаётся второй исход: он встаёт
## перед тобой, ревёт и секунду стоит. Этой секунды хватает, чтобы побежать.
const DIVE_BLANK := 0.7
## КЛЕТОК ВПЕРЕДИ, ЕСЛИ ВЫХОДИТ НЕ ВПЛОТНУЮ. Было три-шесть — это от двенадцати
## до двадцати четырёх метров, то есть «вылез где-то там». Одна-две клетки: он
## выходит на дистанции, с которой дотянется в пару шагов.
const DIVE_FRONT := [1, 2]
...
## ИДЁТ ЛИ СЕЙЧАС ЧУЖОЙ ПРИЁМ. Ставит мир каждый кадр: хват, подтягивание,
## бросок, руки с потолка, «замри». Тварь сама о них не знает, а прерывать их
## нырком нельзя.
var busy: bool = false
...
## НАСКОЛЬКО ОНА СВЕТИТСЯ. Одна ручка на всё её свечение: кромка тела, руки,
## щупальца. Единица — как было: на двенадцати метрах она читалась зелёным
## комом, то есть подсвеченной моделью, а не формой в темноте. Пробуем убрать
## свет и оставить очертание — по замеру стенда «силуэт» видно, что меняется.
const GLOW := 0.25
## И НАСКОЛЬКО КРОМКА ГОРИТ СИЛЬНЕЕ ВДАЛИ. Это делалось нарочно — чтобы форму
## было видно из темноты, — но именно оно и превращало её в зелёный фонарь на
## двенадцати метрах. Оставляем немного: очертание есть, свечения нет.
const GLOW_FAR := 0.30
## Цвет кромки: приглушённый зелёный. Ядовитый сам по себе читается лампой,
## этот — мокрой кожей, поймавшей отсвет.
const RIM_TINT := Color(0.10, 0.27, 0.18)
...
func _next_plant(arm: Dictionary, ai: int, fwd: Vector3, on_wall: bool, foot_y: float) -> Vector3:
	...
	# РАЗБРОС ШИРЕ, ЧЕМ БЫЛ. 0.86–1.14 — это почти одинаковый вынос у всех
	# восьми: опоры вставали ровным кольцом, и в покое силуэт читался ребристым
	# цилиндром — «горшком», из которого торчит тело. Теперь одна опора может
	# стоять вдвое ближе другой, и кольца не получается ни при каком повороте.
	spread *= 0.58 + 0.85 * fmod(float(ai) * 0.37, 1.0)
	# И ПО КРУГУ ОНИ СТОЯТ НЕРОВНО: каждой свой доворот, иначе восемь
	# одинаковых промежутков сами собой собираются обратно в частокол.
	var кос: float = sin(float(ai) * 2.39) * 0.42
	...
	# Доворот вокруг своей оси — тот самый разлад в расстановке опор.
	dd = dd.rotated(Vector3.UP, кос)
...
func _pose_tuck(arm: Dictionary) -> void:
	# ВОСЕМЬ ОДИНАКОВЫХ РУК ПО КРУГУ — ЭТО ЧАСТОКОЛ.
	#
	# Руки в покое укладывались по одному правилу: одна высота, один вынос,
	# разница только в фазе покачивания. На кадре выходил ребристый цилиндр с
	# комом наверху, и играющий (21.09) назвал это точно: «кактус в горшке».
	# В покое силуэт читался предметом, а не существом.
	#
	# Теперь у каждой руки свои высота, вынос и судьба: часть держится над
	# телом, часть отставлена вбок, треть завалена вниз и волочится. Круга
	# больше нет ни на одном кадре.
	var y: float = float(arm["yaw"])
	var ph: float = float(arm["ph"])
	var r: Vector3 = Vector3(sin(y), 0.0, cos(y))
	# Три непохожих числа из одной фазы — свой набор на каждую руку.
	var a: float = fposmod(ph * 0.61, 1.0)
	var b: float = fposmod(ph * 1.37 + 0.31, 1.0)
	var c: float = fposmod(ph * 2.11 + 0.67, 1.0)
	var wob: float = sin(_shiv * 1.7 + ph) * 0.26
	var lift: float = sin(_shiv * 1.15 + ph * 1.7) * 0.42
	var h: float = HEIGHT * lerpf(0.42, 1.16, a) + lift
	var out: float = BLOB_R * lerpf(0.6, 1.8, b) + wob
	var local: Vector3 = Vector3(0.0, h, 0.0) + r * out
	var вниз: bool = c < 0.34
	if вниз:
		# Волочится: кончик ниже брюха и ближе к телу.
		local = Vector3(0.0, HEIGHT * 0.18 + lift * 0.4, 0.0) + r * (BLOB_R * 0.75)
	_pose_arm(arm, local, Vector3.DOWN if вниз else Vector3.UP,
		lerpf(0.55, 0.95, b))
...
func _build_arms() -> void:
	...
	for a in ARMS:
		...
		# РОВНОЕ КОЛЬЦО — ЭТО ГОРШОК.
		#
		# Руки сидели строго через равный угол и торчали строго вниз. Пока их
		# позы считаются каждый кадр, это незаметно; но когда тварь заморожена
		# (а она замирает — в засаде, на паузе, в смотровой стенда), позы не
		# считаются вовсе, и остаётся то, чем они СОБРАНЫ: восемь одинаковых
		# столбов по кругу. Играющий назвал это «кактус в горшке», и он прав —
		# ровное кольцо читается предметом, а не существом.
		#
		# Раскладываем их неровно ещё при сборке: свой угол, своя высота корня,
		# свой завал. Тогда и замершая тварь остаётся тушей, а не вазоном.
		var yaw: float = TAU * float(a) / float(ARMS) + sin(float(a) * 2.39) * 0.34
		var root := Node3D.new()
		root.position = mouth + Vector3(sin(yaw), 0.0, cos(yaw)) * BLOB_R \
			* (0.18 + 0.34 * fmod(float(a) * 0.61, 1.0)) \
			+ Vector3(0.0, BLOB_R * (0.45 - 0.9 * fmod(float(a) * 0.37, 1.0)), 0.0)
		root.rotation = Vector3(
			-0.55 + 1.15 * fmod(float(a) * 0.73, 1.0), yaw,
			-0.35 + 0.7 * fmod(float(a) * 0.29, 1.0))
		body.add_child(root)
...
func _grow_out() -> void:
	...
	# Материалы рождаются в разных местах и в разное время; применяем ручку
	# свечения здесь — это проходной двор всех её появлений.
	_apply_glow()
...
func _tick_out(delta: float, player_pos: Vector3, anger: int, in_finale: bool) -> void:
	...
	if mode == "chase":
		...
		dive_t -= delta
		# ПОРОГ РАССТОЯНИЯ — STANDOFF, А НЕ ПОЛТОРА STANDOFF.
		#
		# Условие писалось, когда нырок был редкостью: «не исчезать у игрока
		# из-под носа». Но STANDOFF — это ровно та дистанция, на которой он и
		# стоит, догнав; полтора STANDOFF он не переступает НИКОГДА. Пока
		# нырок случался раз в восемь секунд, это было незаметно, а теперь, с
		# погоней в камне, оно означало «не ныряет вовсе»: стенд намерил сто
		# процентов времени на виду и ноль выходов.
		#
		# Оставляем главное: если он дотянулся (CATCH_DIST), пусть бьёт, а не
		# растворяется. Всё, что дальше, — законный нырок.
		# И «ПОКА ОН ГОНИТ ТЕБЯ К УБЕЖИЩУ» БОЛЬШЕ НЕ МЕШАЕТ. Запрет писался под
		# старый нырок, который означал «он потерял тебя из виду»: отпускать
		# добычу посреди травли было нельзя. Новый нырок значит ровно обратное
		# — он уходит в камень, чтобы выйти вплотную, — и как раз в травле он
		# страшнее всего: бежишь к свету, а он в стенах со всех сторон.
		# И НЕ ПОСРЕДИ СОБСТВЕННОЙ АТАКИ. Играющий: «если поймал, во время атаки
		# пропадает». Так и было: пока он держит игрока, расстояние до него
		# больше STANDOFF, и нырок считал это законным поводом исчезнуть. Ни
		# хват, ни подтягивание, ни бросок нырком не прерываются — `busy`
		# ставит мир, он один знает про все свои приёмы разом.
		#
		# И порог теперь CATCH_DIST, а не STANDOFF: если он дотянулся — пусть
		# бьёт, а не растворяется у игрока перед носом.
		if (dive_t <= 0.0 and mode == "chase" and visible and roar_t <= 0.0
				and not busy and hold_at == Vector3.ZERO
				and chase_t > 2.0 and _flat_dist(player_pos) > CATCH_DIST):
...
## Применить GLOW ко всем её материалам. Зовётся после сборки тела: материалы
## рождаются в разных местах, и забыть один — значит оставить светиться руку.
func _apply_glow() -> void:
	for m in [shell_mat, tend_mat, arm_mat, reach_mat]:
		if m == null:
			continue
		if m.shader == null:
			continue
		var имя: String = m.shader.resource_path
		m.set_shader_parameter("rim_far", GLOW_FAR)
		# ЦВЕТ КРОМКИ ТУСКЛЕЕ, И ЗАДАН ЧИСЛОМ. Сначала я умножал прежний цвет на
		# коэффициент — а функция зовётся при каждом появлении твари, и цвет
		# гас с каждым разом: замер прыгал, потому что зависел от того, сколько
		# раз она успела вылезти.
		m.set_shader_parameter("rim_tint", RIM_TINT)
		if имя.ends_with("blob.gdshader"):
			m.set_shader_parameter("rim_gain", GLOW)
		else:
			# У щупалец своя ручка того же смысла.
			m.set_shader_parameter("rim_k", GLOW)
